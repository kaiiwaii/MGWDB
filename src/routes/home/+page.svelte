<script lang="ts">
    ...
    function logOut() {
        document.cookie = "token" +'=; Path=/; Domain=127.0.0.1 ; SameSite=None; Expires=Thu, 01 Jan 1970 00:00:01 GMT;';
        goto("/login")
    }
...
<body class="bg-gray-100">
    <nav class="bg-blue-500 p-4">
        <div class="container mx-auto flex flex-col md:flex-row items-center">
            ...
            <button class="relative" on:click={toggleProfileDropdown} on:click={toggleProfileDropdown}>
                <img src="/user-solid.svg" alt="Perfil" class="h-8 w-8 rounded-full cursor-pointer">
                <div class={`absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg', ${showProfileDropdown ? 'block' : 'hidden'}`}>
                    <button on:click={()=>goto("/template")} class="block px-4 py-2 text-gray-800 hover:bg-blue-500 hover:text-white">Edit review template</button>
                    <button on:click={logOut} class="block px-4 py-2 text-gray-800 hover:bg-blue-500 hover:text-white">Log out</button>
                </div>
            </button>
