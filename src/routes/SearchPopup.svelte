<script lang="ts">
    ...
    const TEST_TOKEN =
        "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6MSwidGltZXN0YW1wIjoxNzAwNTg0MTg0LjkwNzgwMjh9.oRpCFHhXYZilNAKhorb2RbWl_7BKrmHxTC9J3mInCJM"; // Replace with your actual token
        ...
        try {
            if (searchTerm != "") {
                ...
                const response = await fetch(
                    `http://127.0.0.1:4321/api/games?token=${TEST_TOKEN}&search=${searchTerm}`,
                    {
                        method: "GET",
        ...
        <div class="bg-white p-4 rounded shadow-lg w-1/2">
                ...
                {#each searchResults as result}
                    <li class="flex items-center mb-4">
                        ...
                        <div>
                            ...
                            <p>
                                Release Date: {result.first_release_date &&
                                    formatReleaseDate(
                            ...
                            <button
                                ...
                                class={gamesToAdd.includes(result) ||
                                addedGamesIds.includes(result.id)
                                    ? "mt-2 bg-gray-500 text-white cursor-not-allowed px-4 py-2 rounded-full"
                                    : "mt-2 bg-green-500 text-white px-4 py-2 rounded-full"}
                                disabled={gamesToAdd.includes(result) ||
                                    addedGamesIds.includes(result.id)}
                            >
                                {gamesToAdd.includes(result) ||
                                addedGamesIds.includes(result.id)
                                    ? "Game Added"
                                    : "Add Game"}
                            </button>
            ...
            {#if isSearching && searchTerm !== ""}
                <div class="flex items-center justify-center mt-4">
                    ...
                    <div
                        class="animate-spin rounded-full h-8 w-8 border-t-2 border-blue-500 border-r-2 border-b-2 border-gray-300"
                    ></div>
                    <span class="ml-2"
                        >{isSearching
                            ? "Searching..."
                            : "Adding selected games..."}</span
                    >
                </div>
            {/if}
