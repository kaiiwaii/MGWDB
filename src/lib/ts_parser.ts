class RatingSystem {
  name: string;
  elements: (Group | Category | Setting)[];
  ...
  parse(jsonobj: Record<string, any>): void {
    let elements = Object.keys(jsonobj);
    let parsed_elements: (Group | Category | Setting)[] = [];
  ...
  private __parse_group(name: string, group: Record<string, any>): Group {
    if ("categories" in group && Object.keys(group["categories"]).length > 0) {
      ...
      for (const [cat, weight] of Object.entries(group["categories"])) {
        let pweight = parseFloat(cat as string);
        if (isNaN(pweight)) {
          throw {
            name: "InvalidWeightError",
...
class Group {
  ...
  constructor(name: string, weight: number, categories: Category[]) {
    this.name = name;
    this.weight = weight;
