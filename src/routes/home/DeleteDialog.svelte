    ...
    async function removeGame() {
        if(!isDeleting) {
            ...
            if(selected_game.saved) {
                ...
                const response = await fetch(`http://127.0.0.1:4321/deletegame?id=${selected_game.game.id}`, {
                    method: 'POST',
                    credentials: "include"
                });
...
<dialog id="delete_dialog" class="fixed inset-1/2 bg-white p-8 rounded-md shadow-lg z-50 transform -translate-x-1/2 -translate-y-1/2 lg:w-[30%] w-[80%]">
    <h1 class="text-xl font-bold mb-4">Do you want to delete the game?</h1>
    <button class="bg-gray-500 text-white px-4 py-2 rounded-md mr-2" on:click={closeDialog}>No</button>
    <button class="bg-red-500 text-white px-4 py-2 rounded-md" on:click={removeGame}>{isDeleting ? 'Deleting...' : 'Yes'}</button>
  </dialog>