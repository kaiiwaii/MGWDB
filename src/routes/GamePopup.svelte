...
<script lang="ts">
  ...
  {#if visible}
    <div class="fixed top-0 left-0 w-full h-full flex items-center justify-center">
      ...
      <div class="relative z-10 p-4 bg-white rounded-md shadow-md w-4/5 h-3/5 overflow-y-auto">
        <button class="absolute top-2 right-2 text-gray-500" on:click={saveAndExit}>Save and Exit</button>
        {#if game}
          <img src={game.cover.url} alt={game.name} class="mx-auto h-64 object-cover rounded-md" />
          ...
          <p class="text-gray-500 mt-2">Release Date: {new Date(game.first_release_date * 1000).toLocaleDateString()}</p>
          <p class="text-gray-500">URL: {game.url}</p>
          <p class="text-gray-500">Platforms: {game.platforms.join(', ')}</p>
          <p class="text-gray-500">Genres: {game.genres.join(', ')}</p>
