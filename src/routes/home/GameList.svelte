...
<script lang="ts">
    ...
    $: filteredGameList = fuzzy.filter(searchTerm, gameList, {extract: g => g.name}).map(el => el.original)

    $: filteredTemporaryGames = fuzzy.filter(searchTerm, temporaryGames, {extract: g => g.name}).map(el => el.original)
